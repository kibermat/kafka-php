...
$$
    begin
        ALTER TABLE er.er_profiles
            ADD COLUMN ext_id bigint default null;
        comment on column er.er_profiles.ext_id is 'Ссылка на внешний идентификатор';
...
$$
    begin
        if not exists(select true from pg_type where typname = 'ext_system_profile_type') then
            --drop type if exists ext_system_profile_type;
            create type kafka.ext_system_profile_type as
            (
                "id"            bigint,
                "name"          text,
                "match_profile" text,
...
drop function if exists kafka.f_ext_profiles8add(pn_ext_id bigint, pu_profile_uid uuid, ps_name text, ps_match_profile text, pu_add_info jsonb)
create function kafka.f_ext_profiles8add(pn_ext_id bigint, pu_profile_uid uuid, ps_name text, ps_match_profile text,
...
$$;
alter function kafka.f_ext_profiles8add( bigint, uuid, text, text, jsonb) owner to dev;


drop function if exists kafka.f_ext_profiles8upd(pn_id bigint, pn_ext_id bigint, pu_profile_uid uuid, ps_name text, ps_match_profile text, pu_add_info jsonb);
create function kafka.f_ext_profiles8upd(pn_id bigint, pn_ext_id bigint, pu_profile_uid uuid, ps_name text,
                                      ps_match_profile text, pu_add_info jsonb) returns bigint
    security definer
    language plpgsql
as
$$
...
$$;
alter function kafka.f_ext_profiles8upd(bigint, bigint, uuid, text, text, jsonb) owner to dev;


drop function if exists kafka.f_ext_profiles8del(pn_id bigint);
create function kafka.f_ext_profiles8del(pn_id bigint) returns void
    security definer
    language plpgsql
as
$$
...
$$;
alter function kafka.f_ext_profiles8del(bigint) owner to dev;


drop function if exists kafka.f_kafka_load_profile(p_topic text);
CREATE OR REPLACE FUNCTION kafka.f_kafka_load_profile(p_topic text)
    RETURNS int AS
$$
DECLARE
    n_cnt      INT DEFAULT 0;
...
BEGIN
    ...
    LOOP
        ...
        with cte as (
            ...
            from jsonb_populate_recordset(null::kafka.ext_system_profile_type,
                                          json_body -> 'response' -> 'Result' -> 'ResultSet') as t
            where t.name is not null
        ),
             ins as (
                 select kafka.f_ext_profiles8add(
                                t.ext_id,
                                uuid_generate_v1(),
                                t.name,
                                t.match_profile,
             ...
             upd as (
                 select kafka.f_ext_profiles8upd(
                                t.id,
                                t.ext_id,
                                p.profile_uid,
                                t.name,
