...
create function er.f_mis_resources8add(pn_ext_id bigint, pu_resource_uid uuid, pn_mo_id bigint, pn_div_id bigint, pn_profile_id bigint, ps_name text, ps_address text, ps_notification text, ps_hint text, pb_is_free boolean, pb_is_paid boolean, pn_price numeric, ps_department text, ps_room text, ps_service text, pn_site_id bigint, ps_emp_sname text, ps_emp_fname text, ps_emp_lname text, pn_record_period integer, pn_time_to_elapse integer, pb_allow_wait_list boolean, ps_wait_list_msg text, pu_add_info jsonb) returns bigint
    security definer
    language plpgsql
as $$
declare
...
create function er.f_mis_resources8upd(pn_id bigint, pn_ext_id bigint, pu_resource_uid uuid, pn_mo_id bigint, pn_div_id bigint, pn_profile_id bigint, ps_name text, ps_address text, ps_notification text, ps_hint text, pb_is_free boolean, pb_is_paid boolean, pn_price numeric, ps_department text, ps_room text, ps_service text, pn_site_id bigint, ps_emp_sname text, ps_emp_fname text, ps_emp_lname text, pn_record_period integer, pn_time_to_elapse integer, pb_allow_wait_list boolean, ps_wait_list_msg text, pu_add_info jsonb) returns bigint
    security definer
    language plpgsql
as $$
declare
...
-- --       where id = n_ext_site_id;
        ...
        with resources as (
            ...
            where
                ...
                (resources.div_id is null or div.id is not null)
        ), cte as (
            select ext.*
              from ext
                   left join er.er_profiles pr ON ( ext.profile_ext_id = pr.ext_id )
                   left join er.er_sites st ON (ext.site_id = st.id )
             where (ext.profile_id is null or pr.id is not null) and
                (ext.site_id is null or st.id is not null)
        ), ins as (
            select er.f_mis_resources8add(
                ...
                where res.id is null and "action_res" = 'add'
            ), upd as (
                ...
                from cte as t
                    left join er.er_resources as res on t.ext_id = res.ext_id
                where res.resource_uid is not null and action_res = 'upd'
            ), cnt as (
