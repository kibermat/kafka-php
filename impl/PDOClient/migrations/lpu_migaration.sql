...
alter table er_mo owner to dev;
...
DO $$
    begin
        ...
        ALTER TABLE er_mo ADD COLUMN ext_id bigint default null;
        comment on column er_mo.ext_id is 'Идентификатор на внешней системе';
        ALTER TABLE er_mo ADD CONSTRAINT fk_ext_id FOREIGN KEY ( ext_id ) REFERENCES ext_entity_values( id ) ON DELETE CASCADE;
...
create function er.f_mis_mo8upd(pn_id bigint, pn_hid bigint, pn_ext_id bigint, pu_mo_uid uuid, ps_code_mo text, ps_mo_name text, ps_full_name text, ps_address text, ps_lat text, ps_long text, pb_without_reg boolean, pb_for_kids boolean, pn_record_period integer, pb_allow_home_call boolean, pu_add_info jsonb) returns bigint
    security definer
    language plpgsql
as $$
declare
...
create function er.f_mis_mo8add( pn_hid bigint, pn_ext_id bigint, pu_mo_uid uuid, ps_code_mo text, ps_mo_name text, ps_full_name text, ps_address text, ps_lat text, ps_long text, pb_without_reg boolean, pb_for_kids boolean, pn_record_period integer, pb_allow_home_call boolean, pu_add_info jsonb) returns bigint
	security definer
	language plpgsql
as $$
declare
